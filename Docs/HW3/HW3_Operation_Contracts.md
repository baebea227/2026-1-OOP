...
| `requestGrab()`<br>`operateLever()`<br>`toggleStageSettingsMenu()` | 플레이어의 오브젝트 잡기, 레버 조작, 설정 메뉴 요청을 처리한다. | 플레이어가 스테이지 안에 있으며 대상 오브젝트나 UI를 조작할 수 있다. | 오브젝트, 구조물, 메뉴 상태가 변경되고 결과가 `updateStageState(result)`로 반환된다. |
| `pushObject()`<br>`operateButton()`<br>`stepOnPressurePlate()` | 플레이어의 협동 밀기, 버튼 조작, 발판 활성화 요청을 처리한다. | 플레이어가 스테이지 안에 있으며 협동 기믹 수행 위치에 있다. | 협동 오브젝트, 다리, 최종 발판 상태가 변경되고 결과가 `updateStageState(result)`로 반환된다. |
