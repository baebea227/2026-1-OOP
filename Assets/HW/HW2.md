...
| **플레이어 이동** | **서버 동기화 성능:** 플레이어의 위치 정보가 서버를 거쳐 다른 클라이언트들에게 실시간에 가깝게 전송되고 화면에 부드럽게 동기화되어야 함. | Performance Efficiency<br>(성능 효율성) | 위치 동기화 지연 시간 **100ms 이하** 및 초당 **60 Tick** 유지 |
